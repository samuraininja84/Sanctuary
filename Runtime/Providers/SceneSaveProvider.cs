...
namespace Sanctuary
{
    ...
    public sealed class SceneSaveProvider : SaveControllerBase
    {
        ...
        protected override void PostInit() => SaveProvider.ConfigureForScene(this, profile, dontDestroyOnLoad);

        public void SetName(string name) => profile.SetFileName(name);

        private void OnDestroy() => SaveProvider.ClearByScope(SaveScope.Scene);
    }
}